...
const wip = path.join(process.cwd(), '..', 'themes', 'living-twilight-color-theme.json');
console.log(wip);
const main = async () => {
    ...
    const generateThemeSVG = (inputPath) => {
        const _code = fs
        ...
        fs.writeFileSync(path.join('assets/code pictures', `${file.slice(file[0] === '.' ? 1 : 0, file.length - (extension.length + 1))}.svg`), svg);
        // fs.writeFileSync(
        // 	path.join(
        // 		'assets/html',
        // 		`${file.slice(
    ...
    fs.writeFileSync(path.join('assets/html', `index.html`), `<html lang="en">
			<head>
				<meta charset="UTF-8" />
